...
import { Baby, Calendar, FileText, Minus, Plus, Sparkles } from 'lucide-react';
...
interface GrowthMeasurementFormProps {
  formId: string;
  measurement?: GrowthHistoryRecord;
...
export function GrowthMeasurementForm({
  ...
  const initialWeight = measurement?.weight ?? (parseFloat(vitals.weight) || 8.6);
  const initialHeight = measurement?.height ?? (parseFloat(vitals.height) || 71.5);
  const initialHeadCirc = measurement?.headCirc ?? (parseFloat(vitals.headCirc) || 44.2);
  ...
  const chartLabels = currentStageData.growthChart?.labels;
  const labels = chartLabels && chartLabels.length > 0 ? chartLabels : fallbackLabels;
  const fallbackLabelIdx = labels.length >= 5 ? 4 : Math.max(0, labels.length - 1);
  const initialLabelIdx = typeof measurement?.labelIndex === 'number'
    ? Math.min(Math.max(0, measurement.labelIndex), Math.max(0, labels.length - 1))
    : fallbackLabelIdx;

  const [date, setDate] = useState(measurement?.date ?? todayStr);
  const [milestoneIdx, setMilestoneIdx] = useState(initialLabelIdx);
  const [weight, setWeight] = useState(initialWeight);
  const [height, setHeight] = useState(initialHeight);
  const [headCirc, setHeadCirc] = useState(initialHeadCirc);
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (measurement) {
      updateGrowthMeasurement(measurement.id, values);
    } else {
      addGrowthMeasurement({ ...values, note: values.note || undefined });
    }

    const metrics = `${weight > 0 ? `${weight}kg` : ''} ${height > 0 ? `• ${height}cm` : ''}`.trim();
    onSaved(`${measurement ? 'Đã cập nhật' : 'Đã lưu'} số đo: ${metrics} 📏`);
  };
    ...
    <form id={formId} onSubmit={handleSubmit} className="tracker-sheet-form growth-sheet-form">
      <div className="tracker-sheet-intro">
        ...
        <div className="tracker-sheet-intro-copy">
          <span className="tracker-sheet-kicker">
            {measurement ? 'CHỈNH SỬA SỐ ĐO' : 'CẬP NHẬT CHỈ SỐ'}
          </span>
          <p>
            {measurement
              ? 'Điều chỉnh lại thông tin của lần cân đo này.'
              : 'Ghi số đo mới để theo dõi nhịp phát triển của bé theo thời gian.'}
          </p>
        </div>
      </div>
