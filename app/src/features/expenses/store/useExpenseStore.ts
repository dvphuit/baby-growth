  ...
  persist(
    (set) => ({
      ...
      deleteExpense: (id) => set((state) => ({
        expenses: state.expenses.filter((record) => record.id !== id),
      })),
      resetTrackingData: () => set({ expenses: [], monthlyBudget: DEFAULT_MONTHLY_BUDGET }),
    }),
    {
      name: 'babygrowth_v4_expenses',
      storage: createJSONStorage(() => indexedDbStorage),
      partialize: (state) => ({ expenses: state.expenses, monthlyBudget: state.monthlyBudget }),
    },
  ),
);
