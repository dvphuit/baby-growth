...
function ownerLabel(owner: 'baby' | 'mom' | 'system'): string {
  return owner === 'baby' ? 'Bé' : owner === 'mom' ? 'Mẹ' : 'Hệ thống';
}
...
export const TimelineView: React.FC<TimelineViewProps> = ({ onOpenAddEntry }) => {
  ...
  const growthHistory = useBabyStore((state) => state.currentStageData().growthHistory);
  ...
  const entries = useMemo(
    ...
    [babyActivities, momActivities, growthHistory]
  );
    ...
    <div className="timeline-view-wrapper">
      <section className="app-card">
        <div className="section-header-row">
          <div>
            <div className="section-eyebrow">NHẬT KÝ</div>
            <h2 className="section-title"><CalendarDays size={17} style={{ verticalAlign: 'middle', marginRight: 6 }} />Lịch sử thực tế</h2>
          </div>
          <button type="button" className="btn-primary-small" onClick={onOpenAddEntry}><Plus size={14} /> Ghi chú</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 10, marginTop: 12 }}>
          ...
          <select aria-label="Lọc người" className="log-input-control" value={ownerFilter} onChange={(event) => setOwnerFilter(event.target.value as typeof ownerFilter)}>
      ...
      <section className="app-card" style={{ marginTop: 12 }}>
        {visibleEntries.length === 0 ? (
          <div className="empty-state" style={{ padding: '24px 4px' }}>
            <p>Không có ghi nhận nào trong ngày này.</p>
          </div>
        ) : (
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                  <div>
                    <div style={{ display: 'flex', gap: 7, alignItems: 'center', marginBottom: 4 }}>
                      <span className="timeline-tag">{ownerLabel(entry.owner)}</span>
                      <strong>{entry.title}</strong>
                    </div>
                    {entry.detail && <div style={{ color: 'var(--color-text-muted)', fontSize: 13 }}>{entry.detail}</div>}
                  </div>
                  <time style={{ whiteSpace: 'nowrap', fontSize: 12, color: 'var(--color-text-muted)' }}>
                    {new Intl.DateTimeFormat('vi-VN', { hour: '2-digit', minute: '2-digit' }).format(new Date(entry.occurredAt))}
                  </time>
                </div>
                {entry.stats.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 9 }}>
                    {entry.stats.map((stat) => <span key={stat} className="timeline-stat-pill">{stat}</span>)}
                  </div>
                )}
              </article>
            ))}
