...
interface ReminderListProps { onQuickLog?: (action: string) => void; }
        ...
        return (
          <div key={occurrence.occurrenceId} className="notification-card-item">
            ...
            <div className="notif-content-box">
              <div className="notif-top-row"><h5 className="notif-title">{occurrence.reminder.title}</h5><span className="notif-badge">{statusLabel(occurrence, now)}</span></div>
              {occurrence.reminder.note && <p className="notif-desc">{occurrence.reminder.note}</p>}
              <span className="notif-time">{new Intl.DateTimeFormat('vi-VN', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }).format(new Date(occurrence.dueAt))}</span>
              {isDue && <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 9 }}>
                ...
                {occurrence.reminder.quickLogAction && <button type="button" className="metric-pill-choice" onClick={() => onQuickLog?.(occurrence.reminder.quickLogAction!)}><Edit3 size={13} /> Ghi nhanh</button>}
              </div>}
            </div>
          </div>
        );
