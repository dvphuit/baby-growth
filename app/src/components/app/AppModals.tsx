  ...
  return (
    <Suspense fallback={<LazyModalFallback />}>
      {modals.isQuickLogOpen && (
        <QuickLogModal
          isOpen={modals.isQuickLogOpen}
          onClose={modals.closeQuickLog}
          onSelectAction={modals.handleQuickAction}
        />
      )}

      {modals.isAddGrowthOpen && (
        <AddGrowthModal
          isOpen={modals.isAddGrowthOpen}
          onClose={modals.closeAddGrowth}
      ...
      {modals.isAddPumpingOpen && (
        <AddPumpingModal
          isOpen={modals.isAddPumpingOpen}
          onClose={modals.closeAddPumping}
          onSuccessToast={onSuccessToast}
        />
      )}
      ...
      {modals.isAddExpenseOpen && (
        <AddExpenseModal
          ...
          onClose={modals.closeAddExpense}
          onSuccessToast={onSuccessToast}
        />
      )}
      ...
      {modals.isAddPostOpen && (
        <AddPostModal
          isOpen={modals.isAddPostOpen}
          onClose={modals.closeAddPost}
          onSuccessToast={onSuccessToast}
          presetTagType={modals.presetPostTagType}
        />
      )}
      ...
      {modals.isAiChatOpen && (
        <AIDoctorChatModal
          isOpen={modals.isAiChatOpen}
          onClose={modals.closeAiChat}
          initialQuestion={modals.aiChatInitialQuestion}
        />
      )}

      {modals.isNotificationOpen && (
        <NotificationModal
          isOpen={modals.isNotificationOpen}
