  ...
  return (
    <div className="haven-home haven-home-mom">
      <section className="haven-summary-card haven-summary-card-mom" aria-labelledby="mom-today-title">
        ...
        <button type="button" className="haven-summary-action" aria-label="+ Hút sữa" onClick={onOpenPumping}>Hút sữa <span>+</span></button>
      </section>
      ...
      <section className="haven-metrics-sheet" aria-label="Tóm tắt chỉ số của Mẹ">
        ...
        <div className="haven-metric-grid">
          <article className="haven-metric-card haven-metric-rose"><span className="haven-metric-icon">◌</span><span className="haven-metric-label">Hút sữa</span><strong>{metrics.pumpingCount ? `${metrics.pumpingAmountMl} ml` : '—'}</strong><small>{metrics.pumpingCount ? `${metrics.pumpingCount} cữ` : 'Chưa ghi nhận'}</small>{metrics.pumpingCount ? <span className="haven-sr-only">{`${metrics.pumpingAmountMl} ml · ${metrics.pumpingCount} cữ`}</span> : null}</article>
          ...
          <article className="haven-metric-card haven-metric-yellow"><span className="haven-metric-icon">☺</span><span className="haven-metric-label">Tâm trạng</span><strong>{latestMood ? moodLabel(latestMood) : '—'}</strong><small>{latestMood ? 'Lần ghi nhận gần nhất' : 'Chưa ghi nhận'}</small></article>
          <article className="haven-metric-card haven-metric-clay"><span className="haven-metric-icon">⌁</span><span className="haven-metric-label">Cữ gần nhất</span><strong>{metrics.lastPumpingAt ? new Intl.DateTimeFormat('vi-VN', { hour: '2-digit', minute: '2-digit' }).format(new Date(metrics.lastPumpingAt)) : '—'}</strong><small>{metrics.lastPumpingAt ? 'Thời điểm ghi nhận' : 'Chưa ghi nhận'}</small></article>
        </div>
      </section>
      ...
      <section className="haven-activity-surface" aria-labelledby="mom-recent-title">
        ...
        {recent.length === 0 ? (
          <div className="haven-empty-state"><span>✦</span><strong>Chưa có khoảnh khắc nào được ghi lại</strong><p>Ghi một hoạt động nhỏ để bắt đầu quan sát nhịp phục hồi của Mẹ.</p><button type="button" className="haven-empty-action" onClick={onOpenPumping}>Ghi hoạt động đầu tiên</button></div>
        ) : (
          <div className="haven-activity-list">
            {recent.map((record) => (<article key={record.id} className="haven-activity-row"><span className="haven-activity-dot haven-activity-dot-mom" aria-hidden="true"></span><div><strong>{activityLabel(record.type)}</strong><p>{new Intl.DateTimeFormat('vi-VN', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }).format(new Date(record.occurredAt))}{record.note ? ` · ${record.note}` : ''}</p></div><span className="haven-chevron">›</span></article>))}
          </div>
        )}
      </section>
    </div>
