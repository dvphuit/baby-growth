...
import { ArrowLeft, Calendar, Droplet, Edit3, Ruler, Scale } from 'lucide-react';
...
export const ProfileView: React.FC<ProfileViewProps> = ({ onOpenEditProfile }) => {
  ...
  const latestGrowth = useMemo(() => currentStageData.growthHistory?.[0] ?? null, [currentStageData.growthHistory]);
  ...
  return (
    <div className="baby-profile-view-container">
      <div className="profile-top-bar">
        <button className="profile-back-btn" onClick={() => navigate('/')} id="btnBackFromProfile">
          <ArrowLeft size={18} /><span>Trang chủ</span>
        </button>
      ...
      <section className="profile-hero-card">
        <div className="profile-hero-content">
          <div className="profile-avatar-wrapper">
            <img src={family.childAvatar} alt={family.childName} className="profile-avatar-img" />
          </div>
        ...
        <div className="profile-medical-card">
          <div className="medical-info-row">
            <div className="medical-info-item">
              <Calendar size={15} />
              <div><span className="medical-item-lbl">Ngày sinh</span><span className="medical-item-val">{formatDateDisplay(family.birthDate)}{family.birthTime ? ` · ${family.birthTime}` : ''}</span></div>
            </div>
          ...
          {family.allergies && family.allergies.length > 0 && (
            <div style={{ marginTop: 12 }}><span className="medical-item-lbl">Dị ứng đã ghi nhận</span><div className="medical-item-val">{family.allergies.join(', ')}</div></div>
          )}
        </div>
      </section>
      ...
      <section className="profile-section-block">
        ...
        <div className="profile-medical-card">
          <div className="medical-info-row">
            <div className="medical-info-item"><div><span className="medical-item-lbl">Mẹ</span><span className="medical-item-val">{family.momName}</span></div></div>
            <div className="medical-info-item"><div><span className="medical-item-lbl">Ba</span><span className="medical-item-val">{family.dadName}</span></div></div>
          </div>
        </div>
      </section>
      ...
      <p style={{ padding: '0 4px 20px', fontSize: 11, color: 'var(--color-text-muted)', lineHeight: 1.5 }}>
        Cung hoàng đạo chỉ là thông tin hồ sơ mang tính giải trí và không được dùng cho đánh giá sức khỏe hay nhắc chăm sóc.
      </p>
    </div>
  );
};
